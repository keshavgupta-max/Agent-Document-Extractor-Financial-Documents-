...
def _create_mock_parsed_document_dict():
    ...
    return {
        ...
        "page_count": 1,
        "pages": [{"page_number": 1, "text": "Sample document text content."}],
        "tables": [],
        "metadata": {},
...
async def test_successful_ingestion_pipeline(mock_registry, tmp_path):
    ...
    assert result.success is True, f"Pipeline failed with error: {result.error_message}"
...
@pytest.mark.asyncio
async def test_parser_failure_short_circuits_downstream(mock_registry, tmp_path):
...
@pytest.mark.asyncio
async def test_validation_failure_stops_embedding_and_storage(mock_registry, tmp_path):
    ...
    val_mock.run = AsyncMock(
        return_value=ToolResult(
            success=True,
            data={"is_valid": False, "errors": ["Invoice total math mismatch"]},
            execution_time_ms=10.0,
        )
    )
...
async def test_query_pipeline_execution(mock_registry):
    ...
    query_mock.run = AsyncMock(
        return_value=ToolResult(
            success=True,
            data={"answer": "The invoice total is $500.", "source_chunks": []},
            execution_time_ms=45.0,
        )
    )

    mock_registry.get.side_effect = (
        lambda name: query_mock if name == "query_documents" else MagicMock()
    )
